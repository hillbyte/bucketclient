...
<script lang="ts">
  ...
  $: filteredFolders = folders.filter(f => 
    f.Prefix?.replace(currentPrefix, '').replace('/', '').toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed)) {
          parsed.forEach(folder => recentlyDeletedFolders.add(folder));
          console.log('Loaded deleted folders from localStorage:', parsed);
        }
      }
    } catch (e) {
  ...
  function saveDeletedFolders() {
    try {
      localStorage.setItem('deletedFolders', JSON.stringify(Array.from(recentlyDeletedFolders)));
    } catch (e) {
  ...
  async function listObjects(prefix: string) {
    ...
    try {
      ...
      const newObjects = response.Contents?.filter(obj => obj.Key && obj.Key !== prefix && !obj.Key.endsWith('/')) || [];
      ...
      console.log('Listed objects:', newObjects.length, 'filtered folders:', newFolders.length);
      console.log('All folder names from API:', response.CommonPrefixes?.map(f => f.Prefix) || []);
      console.log('Filtered folder names:', newFolders.map(f => f.Prefix));
  ...
  function openDeleteModal(item: { Key?: string; Prefix?: string; }, isFolder: boolean) {
    itemToDelete = item;
  ...
  async function deleteFolder(folderPath: string): Promise<boolean> {
    ...
    try {
      ...
      try {
        ...
        console.log('Direct folder marker delete failed, trying recursive delete', e);
      }
      ...
      while (isTruncated) {
        try {
          ...
          if (listedObjects.Contents?.length) {
            allObjects.push(...listedObjects.Contents.map(({ Key, VersionId }) => ({
              Key: Key!,
              // Include version ID if available for versioned deletes
              ...(VersionId ? { VersionId } : {})
            })));
            console.log(`Found ${listedObjects.Contents.length} objects in batch`);
          }
          ...
          console.error('Error listing objects:', e);
          throw new Error(`Failed to list objects in folder: ${e.message}`);
        }
      }
      ...
      if (allObjects.length > 0) {
        ...
        for (let i = 0; i < allObjects.length; i += BATCH_SIZE) {
          ...
          console.log(`Processing delete batch ${batchNum} with ${batch.length} objects`);
          ...
          try {
            ...
            if (response.Errors?.length > 0) {
              console.error(`Errors in batch ${batchNum}:`, response.Errors);
              for (const err of response.Errors) {
                console.error(`Failed to delete ${err.Key}: ${err.Code} - ${err.Message}`);
              }
            } else {
              console.log(`Successfully processed batch ${batchNum} with ${batch.length} objects`);
            }
          } catch (e) {
      ...
      try {
        ...
        console.log('Successfully deleted folder marker after content removal');
      } catch (e) {
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      ...
      if (isBatchDelete && selectedItems.size > 0) {
        console.log(`Processing ${selectedItems.size} selected items for deletion`);
        ...
        console.log(`Found ${foldersToDelete.length} folders and ${filesToDelete.length} files to delete`);
        ...
        for (const folderPath of foldersToDelete) {
          ...
          try {
            ...
            if (!success) {
              console.error(`Failed to delete folder: ${folderPath}`);
        ...
        if (filesToDelete.length > 0) {
          console.log(`Deleting ${filesToDelete.length} individual files in batches`);
          const BATCH_SIZE = 1000;
          let successCount = 0;
          let failCount = 0;
          ...
          for (let i = 0; i < filesToDelete.length; i += BATCH_SIZE) {
            ...
            const batchNum = Math.floor(i / BATCH_SIZE) + 1;
            console.log(`Processing batch ${batchNum} with ${batch.length} files`);
            
            try {
              // Ensure we have valid keys in the batch
              const validBatch = batch.filter(item => item && item.Key);
              ...
              if (response.Deleted?.length) {
                console.log(`Successfully deleted ${response.Deleted.length} files in batch ${batchNum}`);
                successCount += response.Deleted.length;
              }
              
            } catch (e) {
              failCount += Math.min(BATCH_SIZE, filesToDelete.length - i);
              console.error(`Error processing batch ${batchNum}:`, e);
              // Continue with next batch even if one fails
            }
          }
          
          console.log(`Batch delete completed. Success: ${successCount}, Failed: ${failCount}`);
  ...
  async function handleCreateFolder(event: CustomEvent<{ folderName: string }>) {
  ...
  function openRenameModal(item: { Key?: string; Prefix?: string; }, isFolder: boolean) {
    itemToRename = item;
  ...
  function toggleSelectAll(event: Event) {
    ...
    const allVisibleKeys = [
      ...filteredFolders.map(f => f.Prefix).filter(Boolean) as string[],
  ...
  $: allVisibleSelected = (() => {
    ...
    const allSelected = allVisibleKeys.every(key => selectedItems.has(key));
  ...
  async function downloadWithPresignedUrl(key: string) {
    const s3 = getS3Client();
    if (!s3) return;
    const command = new GetObjectCommand({ Bucket: $credentials?.bucketName, Key: key });
  ...
  async function addFolderToZip(zip: JSZip, folderPrefix: string) {
    ...
    for (const obj of objectsInFolder) {
      ...
      const command = new GetObjectCommand({ Bucket: $credentials?.bucketName, Key: obj.Key });
      const url = await getSignedUrl(s3, command, { expiresIn: 60 });
  ...
  async function handleDownload(item: { Key?: string; Prefix?: string }, isFolder: boolean) {
    isDownloading = true;
    ...
    try {
      if (isFolder && item.Prefix) {
        const folderName = item.Prefix.split('/').filter(Boolean).pop() || 'archive';
        const zip = new JSZip();
        ...
        const filesToDownload = allObjects.filter(obj => obj.Key && !obj.Key.endsWith('/'));
        ...
        for (const obj of filesToDownload) {
          ...
          const getObjCommand = new GetObjectCommand({ Bucket: creds.bucketName, Key: obj.Key });
          const response = await s3.send(getObjCommand);
          const blob = await response.Body?.transformToByteArray();
          if (blob) {
            const relativePath = obj.Key!.substring(item.Prefix.length);
            zip.file(relativePath, blob);
          }
        }
  ...
  async function handleBatchDownload() {
    ...
    try {
      ...
      for (const itemKey of selectedItems) {
        processedCount++;
        const isFolder = filteredFolders.some(f => f.Prefix === itemKey);
        
        try {
          if (isFolder) {
            const folderName = itemKey.split('/').filter(Boolean).pop() || 'folder';
      ...
      saveAs(content, `bucket-download-${new Date().toISOString().slice(0, 10)}.zip`);
    ...
    try {
      if (isFolderRename) {
        ...
        if (listedObjects.Contents) {
          ...
          const command = new DeleteObjectsCommand(deleteCommand.input);
          command.middlewareStack.add(
            (next) => async (args) => {
              args.request.headers['Content-MD5'] = calculateContentMd5(deleteCommand.input.Delete);
              return next(args);
            },
            { step: 'build' }
          );
        ...
        keyParts.pop();
        const newKey = (keyParts.length > 0 ? keyParts.join('/') + '/' : '') + newName;
...
{#if showRenameModal && itemToRename}
  <RenameModal
    currentName={isFolderRename ? itemToRename.Prefix?.split('/').filter(p => p).pop() || '' : itemToRename.Key?.split('/').pop() || ''}
...
{#if isDownloading}
  <div class="fixed bottom-4 right-4 bg-surface1 p-4 rounded-lg shadow-lg z-50">
  ...
  <div class="container mx-auto">
    ...
    <header class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
      <div class="flex items-center gap-2">
        <i class="fa-solid fa-box-archive text-3xl sm:text-4xl text-mauve"></i>
        <h1 class="text-3xl sm:text-4xl font-bold text-mauve">BucketClient</h1>
      </div>
      <div class="flex items-center gap-2 flex-wrap justify-center sm:justify-end">

        
        
        <button on:click={disconnect} class="btn btn-sm sm:btn-md bg-red text-white hover:bg-maroon">Disconnect</button>
      </div>
    </header>

    <!-- Search and Filter Bar -->
    <div class="flex flex-col sm:flex-row gap-4 mb-4">
      <input type="text" placeholder="Search files and folders..." bind:value={searchTerm} class="input input-bordered w-full bg-surface0" />
      <select bind:value={filterType} class="select select-bordered w-full sm:w-auto bg-surface0">
    ...
    <main class="p-4 sm:p-6 bg-mantle rounded-lg shadow-lg">
      ...
      {#if selectedItems.size > 0}
        <div class="flex items-center justify-between gap-4 mb-4 p-3 bg-surface0 rounded-lg">
          <span class="text-text">{selectedItems.size} item(s) selected</span>
          <div>
            <button on:click={handleBatchDeleteClick} class="btn btn-sm bg-red text-white hover:bg-maroon">
              <i class="fa-solid fa-trash mr-2"></i>Delete
            </button>
            <button on:click={handleBatchDownload} class="btn btn-sm bg-green text-white hover:bg-teal" disabled={isDownloading}>
      ...
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mb-4 text-sm sm:text-lg text-subtext1">
        <div class="flex items-center gap-4">
          <div class="join">
            <button on:click={goBack} class="btn btn-sm sm:btn-md join-item" disabled={historyIndex <= 0} title="Back"><i class="fa-solid fa-arrow-left"></i></button>
            <button on:click={goForward} class="btn btn-sm sm:btn-md join-item" disabled={historyIndex >= history.length - 1} title="Forward"><i class="fa-solid fa-arrow-right"></i></button>
          </div>
          <div class="flex items-center whitespace-nowrap overflow-x-auto">
            <button class="hover:text-mauve" on:click={() => navigateTo('')}>{$credentials?.bucketName}</button>
            {#each getBreadcrumbs() as crumb}
              <span class="mx-2">/</span>
              <button class="hover:text-mauve" on:click={() => navigateTo(crumb.prefix)}>{crumb.name}</button>
            {/each}
          </div>
        </div>
        <div class="flex items-center gap-2 justify-end pt-2 sm:pt-0">
          <div class="join">
            <button on:click={() => viewMode = 'list'} class="btn btn-sm sm:btn-md join-item" class:btn-active={viewMode === 'list'} title="List View"><i class="fa-solid fa-list"></i></button>
            <button on:click={() => viewMode = 'grid'} class="btn btn-sm sm:btn-md join-item" class:btn-active={viewMode === 'grid'} title="Grid View"><i class="fa-solid fa-grip"></i></button>
          </div>
          <button on:click={() => showNewFolderModal = true} class="btn btn-sm sm:btn-md bg-blue text-white hover:bg-sapphire">
            <i class="fa-solid fa-folder-plus"></i>
          </button>
          <button on:click={() => showUploadModal = true} class="btn btn-sm sm:btn-md bg-green text-white hover:bg-teal">
      ...
      <div class="mt-4 mb-2 p-2 bg-surface0 rounded-md text-sm text-subtext0 overflow-x-auto whitespace-nowrap">
        <span class="font-bold">/</span> {currentPrefix ? `${$credentials?.bucketName}/${currentPrefix}` : $credentials?.bucketName}
      </div>
      ...
      {:else if filteredObjects.length === 0 && filteredFolders.length === 0}
        <p class="text-subtext1">This folder is empty or no items match your search.</p>
      {:else}
        {#if viewMode === 'list'}
                    <div class="p-3 bg-surface0 rounded-lg mb-2 hidden sm:flex items-center">
            <input type="checkbox" class="checkbox mr-3" on:change={toggleSelectAll} checked={allVisibleSelected} />
            <span>Select All</span>
          </div>
          <ul class="space-y-2">
            <!-- Folder List -->
            {#each filteredFolders as folder (folder.Prefix)}
              <li class="flex items-center justify-between p-3 bg-surface0 rounded-lg hover:bg-surface1 transition-colors duration-200">
                                <div class="flex items-center gap-3">
                  <input type="checkbox" class="checkbox" checked={isSelected(folder.Prefix)} on:change={handleCheckboxChange(folder.Prefix)} />
                                    <div class="flex items-center gap-3 cursor-pointer overflow-hidden" on:click={() => navigateTo(folder.Prefix || '')} on:keydown={(e) => handleKeydown(e, () => navigateTo(folder.Prefix || ''))} role="button" tabindex="0">
                    <i class="fa-solid fa-folder text-yellow text-xl"></i>
                    <span class="truncate">{folder.Prefix?.replace(currentPrefix, '').replace('/', '')}</span>
                  </div>
                </div>
                <div class="flex items-center gap-3">
                  <button on:click|stopPropagation={() => handleDownload(folder, true)} class="text-subtext0 hover:text-blue transition-colors"><i class="fa-solid fa-download"></i></button>
                  <button on:click|stopPropagation={() => openRenameModal(folder, true)} class="text-subtext0 hover:text-yellow transition-colors"><i class="fa-solid fa-pen-to-square"></i></button>
                  <button on:click|stopPropagation={() => openDeleteModal(folder, true)} class="text-subtext0 hover:text-red transition-colors"><i class="fa-solid fa-trash"></i></button>
                </div>
              </li>
            {/each}
            <!-- File List -->
            {#each filteredObjects as object (object.Key)}
              <li class="flex items-center justify-between p-3 bg-surface0 rounded-lg hover:bg-surface1 transition-colors duration-200">
                                <div class="flex items-center gap-3">
                  <input type="checkbox" class="checkbox" checked={isSelected(object.Key)} on:change={handleCheckboxChange(object.Key)} />
                                    <div class="flex items-center gap-3 overflow-hidden cursor-pointer" on:click={() => openPreviewModal(object)} on:keydown={(e) => handleKeydown(e, () => openPreviewModal(object))} role="button" tabindex="0">
                    <i class="fa-solid fa-file text-text text-xl"></i>
                    <span class="truncate">{object.Key?.replace(currentPrefix, '')}</span>
                  </div>
                </div>
                <div class="flex items-center gap-3">
                  <button on:click={() => handleDownload(object, false)} class="text-subtext0 hover:text-blue transition-colors"><i class="fa-solid fa-download"></i></button>
                  <button on:click={() => openRenameModal(object, false)} class="text-subtext0 hover:text-yellow transition-colors"><i class="fa-solid fa-pen-to-square"></i></button>
                  <button on:click={() => openShareModal(object)} class="text-subtext0 hover:text-green transition-colors"><i class="fa-solid fa-share-nodes"></i></button>
                  <button on:click={() => openDeleteModal(object, false)} class="text-subtext0 hover:text-red transition-colors"><i class="fa-solid fa-trash"></i></button>
                </div>
              </li>
            {/each}
          </ul>
        {:else}
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4">
            <!-- Folder Grid -->
            {#each filteredFolders as folder (folder.Prefix)}
              <div class="relative flex flex-col items-center justify-center p-4 bg-surface0 rounded-lg hover:bg-surface1 transition-all duration-200 group grid-item">
                <div class="absolute top-2 left-2 z-10 flex items-center">
                  <input 
                    type="checkbox" 
                    class="checkbox" 
                    checked={isSelected(folder.Prefix)} 
                    on:change={handleCheckboxChange(folder.Prefix)}
                    on:click|stopPropagation
                  />
                </div>
                <div class="w-full h-full" on:click={() => navigateTo(folder.Prefix || '')} on:keydown={(e) => handleKeydown(e, () => navigateTo(folder.Prefix || ''))} role="button" tabindex="0">
                  <i class="fa-solid fa-folder text-yellow text-5xl mb-2"></i>
                <span class="text-center break-all w-full">{folder.Prefix?.replace(currentPrefix, '').replace('/', '')}</span>
                </div>
                <div class="absolute bottom-2 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button on:click|stopPropagation={() => handleDownload(folder, true)} class="text-subtext0 hover:text-blue transition-colors p-1 bg-base/50 rounded"><i class="fa-solid fa-download"></i></button>
                  <button on:click|stopPropagation={() => openRenameModal(folder, true)} class="text-subtext0 hover:text-yellow transition-colors p-1 bg-base/50 rounded"><i class="fa-solid fa-pen-to-square"></i></button>
                  <button on:click|stopPropagation={() => openDeleteModal(folder, true)} class="text-subtext0 hover:text-red transition-colors p-1 bg-base/50 rounded"><i class="fa-solid fa-trash"></i></button>
                </div>
              </div>
            {/each}
            <!-- File Grid -->
            {#each filteredObjects as object (object.Key)}
              <div class="relative flex flex-col items-center justify-center p-4 bg-surface0 rounded-lg hover:bg-surface1 transition-all duration-200 group grid-item">
                ...
                <div class="w-full h-full" on:click={() => openPreviewModal(object)} on:keydown={(e) => handleKeydown(e, () => openPreviewModal(object))} role="button" tabindex="0">
                  <i class="fa-solid fa-file text-blue text-5xl mb-2"></i>
                  <span class="text-center break-all w-full">{object.Key?.replace(currentPrefix, '')}</span>
                </div>
                <div class="absolute bottom-2 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button on:click|stopPropagation={() => handleDownload(object, false)} class="text-subtext0 hover:text-blue transition-colors p-1 bg-base/50 rounded"><i class="fa-solid fa-download"></i></button>
                  <button on:click|stopPropagation={() => openRenameModal(object, false)} class="text-subtext0 hover:text-yellow transition-colors p-1 bg-base/50 rounded"><i class="fa-solid fa-pen-to-square"></i></button>
                  <button on:click|stopPropagation={() => openShareModal(object)} class="text-subtext0 hover:text-green transition-colors p-1 bg-base/50 rounded"><i class="fa-solid fa-share-nodes"></i></button>
                  <button on:click|stopPropagation={() => openDeleteModal(object, false)} class="text-subtext0 hover:text-red transition-colors p-1 bg-base/50 rounded"><i class="fa-solid fa-trash"></i></button>
                </div>
              </div>
            {/each}
          </div>
